...
public:
	...
	void writeResults(int g=5) {
		...
		int *res = new int[no[g]];
		for (int i=0;i<no[g];i++ ) {
			res[i] = 0;
		}
		for (int i=0;i<n;i++) {
...
void calc2(string file,int64* gco,double *normal) {
	clock_t startTime, endTime;
	int n  = 0;
	fstream fin;
	...
	fprintf(stderr,"%d %.2f\n", n, (double)elapsed/CLOCKS_PER_SEC);
...
void computeProduct(double* gco1,double* gco2,double &result) {
	...
	result = exp(-0.01*sqrt(result));
}

void computeKernel(const string &directory,const string &output) {
	clock_t startTime, endTime;
	vector<string> m;
	...
	for(vector<string>::iterator i=m.begin(); i != m.end(); ++i) {
		...
		calc(*i,gco,normal);
		string filename = output+basename((char*)(*i).c_str());
		fstream fout;
...
void timing(const string &directory) {
	vector<string> m;
	vector<double*> counts;
	getFilesInDirectory(m,directory);
